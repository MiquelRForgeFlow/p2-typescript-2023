  ...
  export const loadPokemons = async (n: number) => {
    ...
    for (let i = 1; i <= n; i++) {
        try {
            ...
            // Gender-difference forms aren't listed as separate index cards:
            // gender is conveyed by the Gender row and as gender branches in
            // the evolution chain, matching how the detail page treats them.
            const varieties = (speciesData.varieties || []).filter((v: any) => {
              if (v.is_default) return false;
              const s = suffixOf(v.pokemon.name, codename);
